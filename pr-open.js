...
javascript: (function () {
  ...
  const cleanTitle = (value) =>
    normalizeWhitespace((value || "").replace(/\s*Edit title\s*/gi, "").replace(/\s*#\d+\s*$/, ""));

  const getIssueTitle = () => {
    const titleBdi = document.querySelector('h1[data-component="PH_Title"] bdi');
    if (titleBdi) return cleanTitle(titleBdi.textContent);
    const newTitle = document.querySelector('h1[data-component="PH_Title"]');
    if (newTitle) return cleanTitle(newTitle.textContent);
    const legacyTitle = document.querySelector("bdi.js-issue-title");
    if (legacyTitle) return cleanTitle(legacyTitle.textContent);
    return "";
  };
